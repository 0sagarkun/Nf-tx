...
#  CONFIG
# ══════════════════════════════════════════════════════════════════════
BOT_TOKEN        = "8255766514:AAEuRCaCG85t6WtfjfyOYLFCFL_hUdu8uiY"
ADMIN_IDS        = [8033719088]
...
# Unique spinner — neon block style
SPINNERS = ["█▒▒▒▒","▒█▒▒▒","▒▒█▒▒","▒▒▒█▒","▒▒▒▒█","▒▒▒█▒","▒▒█▒▒","▒█▒▒▒"]
PHASES   = [
    ("🔍", "ꜱᴄᴀɴɴɪɴɢ ᴀᴄᴄᴏᴜɴᴛ ᴠᴀᴜʟᴛ"),
    ("🔑", "ᴘɪᴄᴋɪɴɢ ᴀ ᴘʀᴇᴍɪᴜᴍ ᴄᴏᴏᴋɪᴇ"),
    ("🌐", "ᴄᴏɴɴᴇᴄᴛɪɴɢ ᴛᴏ ɴᴇᴛғʟɪx"),
    ("📡", "ꜱᴜʙᴍɪᴛᴛɪɴɢ ᴛᴠ ᴄᴏᴅᴇ"),
    ("✨", "ᴡʀᴀᴘᴘɪɴɢ ᴜᴘ..."),
]
...
async def animate(context, chat_id, msg_id, stop_event, tv_code):
    ...
    while not stop_event.is_set():
        ...
        emoji, phase = PHASES[min(i // 12, len(PHASES) - 1)]
        try:
            await context.bot.edit_message_text(
                chat_id=chat_id, message_id=msg_id,
                text=(f"{emoji}  <b>{phase}</b>\n"
                      f"<code>[{spin}]</code>\n\n"
                      f"📺 TV Code: <code>{tv_code}</code>\n"
                      f"<i>This takes 5–15 seconds. Hang tight 🙏</i>"),
                parse_mode=ParseMode.HTML)
        except Exception: pass
        i += 1; await asyncio.sleep(0.45)

TIPS = [
    "💡 ᴛɪᴘ: TV codes expire in ~15 min. Activate fast!",
    "💡 ᴛɪᴘ: Upload your own cookie for instant private access.",
    "💡 ᴛɪᴘ: You can store up to 5 cookies in your private vault.",
    "💡 ᴛɪᴘ: Just type your 8-digit code anytime — no buttons needed!",
    "💡 ᴛɪᴘ: Use /code 12345678 for the fastest one-command login.",
    "💡 ᴛɪᴘ: Credits reset every 12 hours. Come back for more!",
    "💡 ᴛɪᴘ: Check /stats to see your all-time activation record.",
]

def vault_bar(alive: int) -> str:
    """Show a visual health bar for the vault."""
    if alive >= 500:   filled, label = 5, "🟢 ᴇxᴄᴇʟʟᴇɴᴛ"
    elif alive >= 200: filled, label = 4, "🟢 ɢᴏᴏᴅ"
    elif alive >= 50:  filled, label = 3, "🟡 ᴍᴏᴅᴇʀᴀᴛᴇ"
    elif alive >= 10:  filled, label = 2, "🟠 ʟᴏᴡ"
    elif alive > 0:    filled, label = 1, "🔴 ᴄʀɪᴛɪᴄᴀʟ"
    else:              filled, label = 0, "⚫ ᴇᴍᴘᴛʏ"
    bar = "█" * filled + "░" * (5 - filled)
    return f"[{bar}] {label}"

def get_streak(user_id) -> int:
    c = db(); row = c.execute("SELECT streak_days FROM users WHERE user_id=?", (user_id,)).fetchone(); c.close()
    return row["streak_days"] if row else 0

def update_streak(user_id):
    today = datetime.now().strftime("%Y-%m-%d")
    c = db()
    row = c.execute("SELECT streak_days,last_success_date FROM users WHERE user_id=?", (user_id,)).fetchone()
    if not row: c.close(); return
    streak, last_date = row["streak_days"], row["last_success_date"] or ""
    from datetime import date, timedelta
    yesterday = (date.today() - timedelta(days=1)).strftime("%Y-%m-%d")
    if last_date == today:
        c.close(); return                      # already counted today
    elif last_date == yesterday:
        streak += 1                             # consecutive day
    else:
        streak = 1                              # streak broken
    c.execute("UPDATE users SET streak_days=?,last_success_date=? WHERE user_id=?", (streak, today, user_id))
    c.commit(); c.close()

def streak_badge(days: int) -> str:
    if days >= 30: return f"🏆 {days}d ꜱᴛʀᴇᴀᴋ"
    if days >= 7:  return f"🔥 {days}d ꜱᴛʀᴇᴀᴋ"
    if days >= 3:  return f"⚡ {days}d ꜱᴛʀᴇᴀᴋ"
    if days >= 1:  return f"✨ {days}d ꜱᴛʀᴇᴀᴋ"
    return ""

def build_success_msg(tv_code, info, source, login_no):
    country = info.get("country","Unknown"); flag = get_flag(country)
    def v(k): val = info.get(k,""); return val if val and val != "Unknown" else "—"
    plan  = v("plan");  price = v("price")
    qual  = v("video_quality"); streams = v("max_streams")
    pay   = v("payment_method"); last4 = v("last4")
    since = v("member_since"); billing = v("next_billing")
    src_label = "👤 Your Cookie" if source == "own" else "🏦 Shared Vault"
    return (
        f"╔══ ✅ ᴀᴄᴛɪᴠᴀᴛɪᴏɴ #<b>{login_no}</b> ꜱᴜᴄᴄᴇᴇᴅᴇᴅ ══╗\n\n"
        f"📺  TV Code  » <code>{tv_code}</code>\n"
        f"🌍  Country  » <b>{country} {flag}</b>\n"
        f"📦  Plan     » <b>{plan}</b>  ({price})\n"
        f"🎥  Quality  » <b>{qual}</b>   Screens: <b>{streams}</b>\n"
        f"💳  Payment  » <b>{pay}</b>"
        + (f"  •••• <b>{last4}</b>" if last4 != "—" else "") + "\n"
        f"📅  Member   » <b>{since}</b>\n"
        f"🔄  Renews   » <b>{billing}</b>\n"
        f"📧  Email    » <b>{v('email')}</b>\n\n"
        f"🔗  Source   » {src_label}\n"
        f"╚══════════════════════╝\n\n"
        f"🍿 <b>Sit back and enjoy Netflix!</b>  /start"
    )
...
def main_kb():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🚀 ᴀᴄᴛɪᴠᴀᴛᴇ ᴛᴠ",  callback_data="activate_menu"),
         InlineKeyboardButton("🍪 ᴍʏ ᴄᴏᴏᴋɪᴇꜱ", callback_data="my_cookies")],
...
def activate_menu_kb():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("⚡ ꜰʀᴇᴇ ᴀᴄᴛɪᴠᴀᴛᴇ",           callback_data="free_activate")],
        [InlineKeyboardButton("🍪 ᴜꜱᴇ ᴍʏ ᴏᴡɴ ᴄᴏᴏᴋɪᴇ",       callback_data="own_cookie_menu")],
...
#  START  /start
# ══════════════════════════════════════════════════════════════════════
def _start_text(user_id, first_name):
    alive     = count_vault()
    tip       = random.choice(TIPS)
    streak    = get_streak(user_id)
    sbadge    = streak_badge(streak)
    vbar      = vault_bar(alive)
    own_count = count_user_cookies(user_id)

    lines = [
        f"🎬 <b>ɴᴇᴛғʟɪx ᴛᴠ ᴀᴄᴛɪᴠᴀᴛᴏʀ</b>",
        f"",
        f"👤 ʜᴇʏ, <b>{first_name}</b>!",
        f"🆔 <code>{user_id}</code>"
        + (f"  {sbadge}" if sbadge else ""),
        f"",
        f"🏦 <b>ᴠᴀᴜʟᴛ ꜱᴛᴀᴛᴜꜱ</b>",
        f"   <code>{vbar}</code>  ({alive} ᴄᴏᴏᴋɪᴇꜱ)",
    ]

    if own_count:
        lines.append(f"🍪 <b>Your cookies:</b> {own_count}/5 ready to use")

    lines += [
        f"",
        f"━━━━━━━━━━━━━━━━━━━━",
        f"🚀  ᴀᴄᴛɪᴠᴀᴛᴇ ɴᴇᴛғʟɪx ᴏɴ ᴀɴʏ ᴛᴠ",
        f"🍪  ᴍᴀɴᴀɢᴇ ʏᴏᴜʀ ᴘʀɪᴠᴀᴛᴇ ᴄᴏᴏᴋɪᴇꜱ",
        f"📊  ᴛʀᴀᴄᴋ ᴀᴄᴛɪᴠᴀᴛɪᴏɴ ʜɪꜱᴛᴏʀʏ & ꜱᴛʀᴇᴀᴋ",
        f"⚡  ᴛʏᴘᴇ ᴀɴʏ 8-ᴅɪɢɪᴛ ᴄᴏᴅᴇ — ᴡᴇ ᴄᴀᴛᴄʜ ɪᴛ",
        f"━━━━━━━━━━━━━━━━━━━━",
        f"",
        f"<i>{tip}</i>",
    ]
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_banned(user.id):
        await update.message.reply_text(
            "🚫 <b>ʏᴏᴜ'ʀᴇ ʙᴀɴɴᴇᴅ</b> from this bot.\n"
            f"Think it's a mistake? Contact @{SUPPORT_USERNAME}.",
            parse_mode=ParseMode.HTML); return

    await update.message.reply_text(
        _start_text(user.id, user.first_name or "there"),
...
# ══════════════════════════════════════════════════════════════════════
#  /code COMMAND  — fastest possible login, one line
# ══════════════════════════════════════════════════════════════════════
async def cmd_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Usage: /code 1234-5678  (or any 8-digit format)"""
    user = update.effective_user
    ensure_user(user.id, user.first_name or "", user.username or "")
    ...
    if len(tv_code) != 8:
        await update.message.reply_text(
            "⚡ <b>/code — ɪɴꜱᴛᴀɴᴛ ᴀᴄᴛɪᴠᴀᴛɪᴏɴ</b>\n\n"
            "Usage:\n"
            "  <code>/code 1234-5678</code>\n"
            "  <code>/code 12345678</code>\n"
            "  <code>/code 1234 5678</code>\n\n"
            "<i>No buttons. No menus. Just send the code.</i>",
            parse_mode=ParseMode.HTML)
        return
...
async def _show_stats(send_fn, user):
    ...
    today_user   = c.execute(
        ...
        (user.id,)).fetchone()["n"]
    user_best_day = c.execute(
        "SELECT COUNT(*) AS n, strftime('%Y-%m-%d', ts, 'unixepoch') AS day "
        "FROM login_log WHERE user_id=? AND success=1 GROUP BY day ORDER BY n DESC LIMIT 1",
        (user.id,)).fetchone()
    with active_lock:
        active_now = sum(1 for t in active_users.values() if now - t <= 300)
    c.close()

    success_rate  = f"{total_acts*100//total_all}%" if total_all else "0%"
    best_day_str  = f"{user_best_day['n']} on {user_best_day['day']}" if user_best_day else "—"
    if unlimited_flag: credit_str = "♾️ Unlimited"
    else: credit_str = f"{credits_left}/{MAX_CREDITS}"

    await send_fn(
        f"📊 <b>ʏᴏᴜʀ ᴅᴀꜱʜʙᴏᴀʀᴅ</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        ...
        f"┌─── 🌍 ɢʟᴏʙᴀʟ ───────────────\n"
        f"│  👥 Users        {total_users}\n"
        f"│  📺 Activations  {total_acts}\n"
        f"│  🎯 Success rate {success_rate}\n"
        f"│  ⚡ Online now   {active_now}\n"
        f"└──────────────────────────────\n\n"
        f"┌─── 👤 ʏᴏᴜʀꜱ ─────────────────\n"
        f"│  ✅ Total wins   {row.get('successful_logins', 0)}\n"
        f"│  📅 Today        {today_user} / {MAX_CREDITS}\n"
        f"│  🔋 Credits      {credit_str}\n"
        f"│  🏆 Best day     {best_day_str}\n"
        f"│  🔥 Streak       {streak}d\n"
        f"└──────────────────────────────\n\n"
        f"<i>⏰ Credits reset in {fmt_seconds(reset_in) if reset_in else 'next window'}</i>",
        parse_mode=ParseMode.HTML,
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("💎 ᴜᴘɢʀᴀᴅᴇ ᴘʟᴀɴ", callback_data="buy_menu"),
             InlineKeyboardButton("🚀 ᴀᴄᴛɪᴠᴀᴛᴇ ɴᴏᴡ", callback_data="activate_menu")],
            [InlineKeyboardButton("🔙 ʙᴀᴄᴋ",         callback_data="back_start")],
        ]),
    )
...
def _help_text():
    return (
        ...
        "│  2. Select <b>Sign In</b> → <b>Use a code</b>\n"
        "│  3. You'll see an 8-digit code\n"
        "│  4. Send that code here (any format)\n"
        "│  5. Your TV activates in seconds ✅\n"
        "└────────────────────────────────────\n\n"
        "⚡ <b>ᴛʜʀᴇᴇ ᴡᴀʏꜱ ᴛᴏ ꜱᴇɴᴅ ᴀ ᴄᴏᴅᴇ</b>\n\n"
        "  🥇 Just type it → <code>12345678</code>\n"
        "       (auto-detected, fastest)\n\n"
        "  🥈 Use /code → <code>/code 1234-5678</code>\n"
        "       (one-command, no menus)\n\n"
        "  🥉 Use buttons → 🚀 Activate TV menu\n"
        "       (guided flow)\n\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        "🍪 <b>ᴜꜱɪɴɢ ʏᴏᴜʀ ᴏᴡɴ ᴄᴏᴏᴋɪᴇꜱ</b>\n\n"
        "  • Upload via 🍪 My Cookies → Add\n"
        "  • Supports .txt and .zip files\n"
        "  • Store up to 5 cookies\n"
        "  • They're used for your activations\n\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        "📋 <b>ᴄᴏᴍᴍᴀɴᴅꜱ</b>\n\n"
        "  /start   — Main menu\n"
        "  /code    — Instant activate\n"
        "  /tv      — Activate (with menu)\n"
...
async def cb_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── My Cookies Manager ────────────────────────────────────────────
    elif data == "my_cookies":
        c = db()
        rows = c.execute(
            "SELECT id, source, added_at FROM user_cookies WHERE user_id=? ORDER BY added_at DESC",
            (user.id,)).fetchall()
        c.close()
        total = len(rows)
        if total == 0:
            await q.edit_message_text(
                "🍪 <b>ᴍʏ ᴄᴏᴏᴋɪᴇ ᴠᴀᴜʟᴛ</b>\n\n"
                "Your vault is empty. Upload a cookie file to get started!\n\n"
                "📁 Supports .txt and .zip\n"
                "🔒 Stored privately — only you can use them",
                parse_mode=ParseMode.HTML,
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("➕ ᴀᴅᴅ ᴄᴏᴏᴋɪᴇ", callback_data="own_cookie_menu")],
                    [InlineKeyboardButton("🔙 ʙᴀᴄᴋ",        callback_data="back_start")],
                ]))
            return

        slots_bar = "🟩" * total + "⬜" * (MAX_USER_COOKIES - total)
        lines = [
            f"🍪 <b>ᴍʏ ᴄᴏᴏᴋɪᴇ ᴠᴀᴜʟᴛ</b>",
            f"",
            f"Slots: {slots_bar}  <b>{total}/{MAX_USER_COOKIES}</b>",
            f"",
        ]
        buttons = []
        for i, r in enumerate(rows, 1):
            ts = datetime.fromtimestamp(r["added_at"]).strftime("%b %d %H:%M")
            src = (r["source"] or "cookie")[:22]
            lines.append(f"<b>{i}.</b> 📄 <code>{src}</code>  <i>({ts})</i>")
            buttons.append([
                InlineKeyboardButton(f"🗑 Delete #{i}", callback_data=f"del_ucookie_{r['id']}")
            ])
        lines.append("")
        lines.append("<i>Tap delete to remove a cookie from your vault.</i>")

        if total < MAX_USER_COOKIES:
            buttons.append([InlineKeyboardButton("➕ ᴀᴅᴅ ᴍᴏʀᴇ", callback_data="own_cookie_menu")])
        buttons.append([
            InlineKeyboardButton("🚀 ᴜꜱᴇ ᴏɴᴇ ɴᴏᴡ", callback_data="own_cookie_menu"),
            InlineKeyboardButton("🔙 ʙᴀᴄᴋ", callback_data="back_start"),
        ])
        await q.edit_message_text("\n".join(lines), parse_mode=ParseMode.HTML,
                                  reply_markup=InlineKeyboardMarkup(buttons))

    elif data.startswith("del_ucookie_"):
        cookie_id = int(data.split("_")[-1])
        c = db()
        row = c.execute("SELECT id FROM user_cookies WHERE id=? AND user_id=?", (cookie_id, user.id)).fetchone()
        if row:
            c.execute("DELETE FROM user_cookies WHERE id=?", (cookie_id,))
            c.commit()
        c.close()
        await q.answer("🗑 Cookie deleted.", show_alert=False)
        # re-render the My Cookies page
        c2 = db()
        rows = c2.execute(
            "SELECT id, source, added_at FROM user_cookies WHERE user_id=? ORDER BY added_at DESC",
            (user.id,)).fetchall()
        ...
        if total == 0:
            ...
            return
        slots_bar = "🟩" * total + "⬜" * (MAX_USER_COOKIES - total)
        lines = [f"🍪 <b>ᴍʏ ᴄᴏᴏᴋɪᴇ ᴠᴀᴜʟᴛ</b>", f"", f"Slots: {slots_bar}  <b>{total}/{MAX_USER_COOKIES}</b>", f""]
        buttons = []
        for i, r in enumerate(rows, 1):
            ts  = datetime.fromtimestamp(r["added_at"]).strftime("%b %d %H:%M")
            src = (r["source"] or "cookie")[:22]
            lines.append(f"<b>{i}.</b> 📄 <code>{src}</code>  <i>({ts})</i>")
            buttons.append([InlineKeyboardButton(f"🗑 Delete #{i}", callback_data=f"del_ucookie_{r['id']}")])
        if total < MAX_USER_COOKIES:
            buttons.append([InlineKeyboardButton("➕ ᴀᴅᴅ ᴍᴏʀᴇ", callback_data="own_cookie_menu")])
        buttons.append([InlineKeyboardButton("🔙 ʙᴀᴄᴋ", callback_data="back_start")])
        await q.edit_message_text("\n".join(lines), parse_mode=ParseMode.HTML,
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Auto-detect: 8 digits sent anywhere (no state needed) ────────
    raw = (msg.text or "").strip()
    # Only digits, dashes, spaces — looks like a TV code
    if raw and re.fullmatch(r"[\d\s\-]{6,12}", raw):
        tv_code = re.sub(r"\D", "", raw)
        if len(tv_code) == 8:
            ensure_user(user.id, user.first_name or "", user.username or "")
            await _run_free_login(update, context, user, tv_code, is_callback=False)
...
def main():
    print("═" * 60)
    print("  Netflix TV Activator — Unique Edition v5.0")
    print("  Auto-detect • Cookie Vault • Streak • /code")
    print("═" * 60)
    init_db()
